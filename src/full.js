...
setInterval(_ => {
    if (t % 500 < 300)
        for (i = 0; i < 256 * 256; i += 1)
            x = i % 256 - 128,
                y = i / 256 - 128,
                I.data[i * 4] = I.data[i * 4 + 1] = I.data[i * 4 + 2] = Math.max(Math.min(Math.abs(x - y), Math.abs(x + y)), Math.max(Math.abs(x - y), Math.abs(x + y)) - 99) % 30 < 20 ? 0 : 256
    else
    ...
    c.filter = 'brightness(' + Math.abs(Math.cos(t / 33 /*yaw*/) * Math.cos(t / 99 /*pitch*/))
    ...
    c.filter = 'brightness(' + Math.abs(Math.sin(t / 33 /*yaw*/) * Math.cos(t / 99 /*pitch*/))
    c.drawImage(n, -128, -128)
